...
public class Valor_referencia_composicao_exameDAO extends ConexaoDB {
	...
	private static final String INSERT_VALOR_REFERENCIA_COMPOSICAO_EXAME_SQL = "INSERT INTO valor_referencia_composicao_exame (valor_minimo, valor_maximo, limitador_minimo, limitador_maximo) VALUES (?, ?);";
	private static final String SELECT_VALOR_REFERENCIA_COMPOSICAO_EXAME_BY_ID = "SELECT id, descricao, observacao FROM valor_referencia_composicao_exame WHERE id = ?";
	...
	private static final String UPDATE_VALOR_REFERENCIA_COMPOSICAO_EXAME_SQL = "UPDATE valor_referencia_composicao_exame SET valor_minimo = ?, valor_maximo = ?, limitador_minimo = ?, limitador_maximo = ? WHERE id = ?;";
	...
	public Valor_referencia_composicao_exame findByDecricao(String descricao) {
		...
		try (PreparedStatement preparedStatement = prepararSQL(BUSCAR_POR_DESCRICAO_VALOR_REFERENCIA_COMPOSICAO_EXAME_SQL)) {
			...
			while (rs.next()) {
				entidade = new Valor_referencia_composicao_exame((int) rs.getLong("id"), rs.getString("valor_minimo"), rs.getString("valor_maximo"), rs.getString("limitador_minimo"), rs.getString("limitador_maximo"));
			}
		} catch (SQLException e) {
			printSQLException(e);
		} catch (ClassNotFoundException e) {
	...
	public Valor_referencia_composicao_exame findById(long id) {
		...
		try (PreparedStatement preparedStatement = prepararSQL(SELECT_VALOR_REFERENCIA_COMPOSICAO_EXAME_BY_ID)) {
			...
			while (rs.next()) {
				...
				entidade = new Valor_referencia_composicao_exame((int) id, valor_minimo, valor_maximo, limitador_minimo, limitador_maximo);
			}
		} catch (SQLException e) {
			printSQLException(e);
		} catch (ClassNotFoundException e) {
	...
	public List<Valor_referencia_composicao_exame> selectAllValor_referencia_composicao_exames() {
		...
		try (PreparedStatement preparedStatement = prepararSQL(SELECT_ALL_VALOR_REFERENCIA_COMPOSICAO_EXAME)) {
			...
			while (rs.next()) {
				...
				String limitador_minimo = rs.getString("limitador_minimo");
				String limitador_maximo = rs.getString("limitador_maximo");
				entidades.add(new Valor_referencia_composicao_exame((int) id, valor_minimo, valor_maximo, limitador_minimo, limitador_maximo));
			}
		} catch (SQLException e) {
			printSQLException(e);
		} catch (ClassNotFoundException e) {
	...
	public void updateValor_referencia_composicao_exame(Valor_referencia_composicao_exame entidade) throws SQLException {
		try (PreparedStatement statement = prepararSQL(UPDATE_VALOR_REFERENCIA_COMPOSICAO_EXAME_SQL)) {
			...
			statement.setString(3, entidade.getLimitador_maximo());
			statement.setString(4, entidade.getLimitador_minimo());
